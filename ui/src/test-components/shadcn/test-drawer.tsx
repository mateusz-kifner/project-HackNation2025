...
function TestDrawer() {
  ...
  const [isDesktop, setIsDesktop] = useState<boolean>(false)
    ...
    const update = (e: MediaQueryListEvent | MediaQueryList) => {
      const isMatch = 'matches' in e ? (e as MediaQueryListEvent).matches : (mediaQueryList as any).matches;
      setIsDesktop( !!isMatch );
    };

    mediaQueryList.addEventListener?.('change', update as EventListener);

    setIsDesktop( !!mediaQueryList.matches );

    return () => {
        mediaQueryList.removeEventListener?.('change', update as EventListener);
    };
  },[])
